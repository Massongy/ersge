...
class ErsgeEnrollment(models.Model):
    ...
    student_id = fields.Many2one(
        "ersge.student", string="Élève", required=True, ondelete="restrict"
    )
    annee_scolaire = fields.Char(string="Année scolaire", required=True)
    ...
    # === PARASCOLAIRE ===
    after_school = fields.Boolean(string="Inscrit au parascolaire", default=False)
    ...
    after_school_amount = fields.Float(
        string="Montant mensuel parascolaire", default=0.0
    )

    # === CRÉNEAUX PARASCOLAIRE ===
    slot_ids = fields.One2many("ersge.slot", "enrollment_id", string="Créneaux")
...
class ErsgeSlot(models.Model):
    ...
    enrollment_id = fields.Many2one(
        "ersge.enrollment", string="Inscription", required=True, ondelete="cascade"
    )
